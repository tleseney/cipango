...
public class SipServer extends ContainerLifeCycle
{
	...
	public final static String CONFIGURATION_ATTR="org.cipango.sipapp.configuration";
	...
	public ClassList setDefaultConfiguration()
    {
		 ClassList cl= getBean(ClassList.class);
         if (cl!=null)
             return cl;
         cl=defaultConfiguration();
         addBean(cl);
         return cl;
    }
	
	public ClassList defaultConfiguration()
    {
        ClassList cl=getBean(ClassList.class);
        if (cl!=null)
            return new ClassList(cl);
        Object attr = _server.getAttribute(CONFIGURATION_ATTR);
        if (attr instanceof ClassList)
            return new ClassList((ClassList)attr);
        if (attr instanceof String[])
            return new ClassList((String[])attr);
        cl = new ClassList();
        cl.add("org.cipango.server.sipapp.SipXmlConfiguration");
        return cl;
    }
