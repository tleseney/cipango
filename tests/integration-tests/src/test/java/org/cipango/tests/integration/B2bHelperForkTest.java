...
public class B2bHelperForkTest extends UaTestCase
{
	...
	public void test3Response100rel() throws Throwable 
	{
		Endpoint bob = createEndpoint("bob");
		UaRunnable replyB = new SessionProgressReply(bob.getUserAgent());
		Endpoint carol = createEndpoint("bob", "carol");
		UaRunnable replyC = new SessionProgressReply(carol.getUserAgent());
		Endpoint dan = createEndpoint("bob", "dan");
		UaRunnable replyD = new SessionProgressReply(dan.getUserAgent());
		replyB.start();
		replyC.start();
		replyD.start();
		Thread.sleep(10);
		...
		request.addAddressHeader(B2bHelperProxyServlet.PROXY_URIS, bob.getContact(), true);
		request.addAddressHeader(B2bHelperProxyServlet.PROXY_URIS, carol.getContact(), true);
		request.addAddressHeader(B2bHelperProxyServlet.PROXY_URIS, dan.getContact(), true);
		request.addHeader(SipHeaders.SUPPORTED, "100rel");
		...
		SipServletResponse response3 = callA.waitForResponse();
		assertThat(response3, hasStatus(SipServletResponse.SC_SESSION_PROGRESS));
		SipSession session3 = response3.getSession();
		...
		_ua.decorate(response1.createPrack()).send();
		_ua.decorate(response2.createPrack()).send();
		_ua.decorate(response3.createPrack()).send();
	    ...
	    response3 = callA.waitForResponse();
	    assertThat(response3, isSuccess());
	    assertThat(response3.getMethod(), is(SipMethods.PRACK));
	    ...
	    response3 = callA.waitForResponse();
        assertThat(response3, isSuccess());
	    assertThat(response3.getMethod(), is(SipMethods.INVITE));
	    _ua.decorate(response3.createAck()).send();
		...
		_ua.decorate(session3.createRequest(SipMethods.BYE)).send();
		assertThat(callA.waitForResponse(), isSuccess());
